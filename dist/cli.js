...
function bucketScopeGrowth2(count) {
  if (count === null || count === void 0 || count < 0) return "unknown";
...
function progressPayload(state, now = /* @__PURE__ */ new Date()) {
  ...
  return {
    ...
    filesModifiedBucket: bucketFileCount2(state.filesModifiedCount),
    ...
    scopeGrowthBucket: bucketScopeGrowth2(state.filesModifiedCount),
...
function serializeAgentRunEventForGraphQL(input) {
  const progressPayload2 = input.progressPayload;
  const serializedProgress = progressPayload2 ? stripUndefined4({
    ...progressPayload2,
