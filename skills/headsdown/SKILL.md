...
Use `/headsdown:timebox <duration>` when the user wants a focused work deadline that is shorter than the current calendar-derived window. Example forms include `30m`, `45m`, `1h`, and `1h30m`.
...
- `/headsdown:timebox 30m` sets or replaces the active local deadline.
